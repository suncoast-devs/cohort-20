...
namespace LINQ
{
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            // Take the scores list and make a new where each score is just one higher
            ...
            Console.WriteLine($"There are {movies.Count} movies in our list");

            var movieNames = movies.Select(movie => movie.Name);
            Console.WriteLine(String.Join(", ", movieNames));

            var tagLines = movies.Select(movie => movie.Tagline);
            Console.WriteLine(String.Join(", ", tagLines));

            var descriptions = movies.Select(movie => $"The movie {movie.Name} is {movie.Tagline} with a ticket price of {movie.PricePerTicket}");
            Console.WriteLine(String.Join(", ", descriptions));

            var popularMovies = movies.Where(movie => movie.Screenings >= 100);
            var popularMovieNames = popularMovies.Select(popularMovie => popularMovie.Name);
            Console.WriteLine(String.Join(", ", popularMovieNames));

            var areAllMoviesOlderThan1965 = movies.All(movie => movie.ReleasedDate.Year < 1965);
            Console.WriteLine(areAllMoviesOlderThan1965);


            var movieNamesAndWhereTheyAreInTheList = movies.Select((movie, theMovieIndex) => $"The movie is {movie.Name} is at index {theMovieIndex}");
            Console.WriteLine();
